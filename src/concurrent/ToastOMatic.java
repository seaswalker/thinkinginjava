...
class ToastQueue extends LinkedBlockingQueue<Toast> {
	...
	private static final long serialVersionUID = -300356898191826779L;
}
...
class Jelly implements Runnable {
	...
	public Jelly(ToastQueue buttered, ToastQueue finished) {
		butteredQueue = buttered;
		finishedQueue = finished;
	}
	...
	public void run() {
		try {
			while (!Thread.interrupted()) {
				...
				if (t.getId() != counter++
						|| (t.getStatus() != Toast.Status.JAMMED && t.getStatus() != Toast.Status.JELLIED)) {
